...
export class GameNotificationsComponent implements OnDestroy, AfterViewInit {
  ...
  @Select(GameState.moves) _moves$: Observable<GameMoveIterface[]>;
  ...
  figure(move: GameMoveIterface): string {
    if (move.san[0] === '0') {
      return '';
    }
  ...
  prepareMove(move: GameMoveIterface): INotationMove {
  ...
  selectMove(move: GameMoveIterface) {
