...
import {
  ...
  OnlineTournamentSortedInterface
...
export class TournamentTimelineComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input()
  networkTournaments: OnlineTournamentInterface[] = [];
  networkTournaments$: BehaviorSubject<OnlineTournamentInterface[]> = new BehaviorSubject([]);
  ...
  fideRatedLines$: BehaviorSubject<OnlineTournamentSortedInterface[][]> = new BehaviorSubject([]);
  wCRatedLines$: BehaviorSubject<OnlineTournamentSortedInterface[][]> = new BehaviorSubject([]);
  ...
  ngOnInit() {
    ...
    this.networkTournaments$.pipe(
      takeUntil(this.destroy$),
      distinctUntilChanged(),
      filter((networkTournaments) => !!networkTournaments)
    ).subscribe((networkTournaments) => {
      const fideRatedLines = [];
      const wCRatedLines = [];

      const networkTournamentsSorted = networkTournaments
        .map((tournament) => {
          return {
            ...tournament,
      ...
      const tournamentsFide = [...networkTournamentsSorted].filter((tournament) => (tournament.rating_type === GameRatingMode.FIDERATED));
      const tournamentsWC = [...networkTournamentsSorted].filter((tournament) => (tournament.rating_type === GameRatingMode.RATED));
  ...
  calcFinishTime(tournament: OnlineTournamentInterface): string {
  ...
  calcComparableFinishTime(tournament: OnlineTournamentInterface): string {
  ...
  calcLines(tournaments: OnlineTournamentSortedInterface[], accumulator) {
    const nextTournaments = [...tournaments];
    let deletedCount;
    let currentLine: OnlineTournamentSortedInterface[];
  ...
  calcTournamentOffset(tournament: OnlineTournamentSortedInterface) {
    return tournament.momentTime.diff(this.startTime, 'm') / 30 * this.sizeOfHalfAnHour;
  }

  calcTournamentWidth(tournament: OnlineTournamentSortedInterface) {
    return Math.abs(moment(tournament.datetime_of_finish).diff(tournament.momentTime, 'm')) / 30 * this.sizeOfHalfAnHour;
  }

  calcTournamentTop(tournament: OnlineTournamentSortedInterface, index: number) {
