...
import { ArchiveGameResult, OnlinePlayerGameArchiveInterface } from '@app/modules/game/state/game-history-response.model';
...
import { OnlinePlayerGameArchiveGroupedByMonthInterface } from '@app/modules/game/models';
...
export class MyGamesComponent implements OnInit, OnDestroy {
  ...
  gamesHistoryGrouped = new BehaviorSubject<OnlinePlayerGameArchiveGroupedByMonthInterface[]>([]);
  gamesList: OnlinePlayerGameArchiveInterface[] = [];
  ...
  public getAnalyzeUrl(url: string): string {
  ...
  groupGames(results) {
    ...
    const tempGroupedList: OnlinePlayerGameArchiveGroupedByMonthInterface[] = [];
    let temp: OnlinePlayerGameArchiveGroupedByMonthInterface = {
      month: null,
      year: null,
      results: []
    };
