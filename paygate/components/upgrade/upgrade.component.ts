...
export class UpgradeComponent implements OnDestroy, OnChanges {
  ...
  popupTitle$ = combineLatest([
    ...
    map(([proPurchased, fidePurchased]) => {
      ...
      if (!title) {
        ...
        ? (this.isFirstTime ? 'PAYGATE.GOT_PRO' : 'PAYGATE.HAVE_PRO' )
        : (this.isFirstTime ? 'PAYGATE.GOT_FREE' : 'PAYGATE.HAVE_FREE');
      }
  ...
  onPopState(): void {
    if (this.location.path() === this.generateMembershipUrlPattern('paygate', 'payment')) {
        this.canUpgrade$.pipe(take(1)).subscribe((canUpgrade: boolean) => {
  ...
  generateMembershipUrlPattern(component: string, type: string): string {
    return `/membership(p:${component}/${type})`;
  }
