...
import { filter, map, finalize, takeUntil } from 'rxjs/operators';
...
export class PaymentComponent implements OnDestroy {
  ...
  form = new PaygateForm({
    card_number: new PaygateFormControl(null, [Validators.required, Validators.minLength(16)]),
    cvc: new PaygateFormControl(null, [Validators.required, Validators.minLength(3)]),
  ...
  title$ = combineLatest([this.proSelected$, this.fideSelected$]).pipe(
  ...
  totalPrice$ = this.paygateService.cart.pipe(map((cart) => cart.total));
  ...
  constructor(
    ...
    Object.keys(this.form.controls).forEach((name) => {
      ...
      this.form.controls[name].valueChanges.pipe(takeUntil(this.destroy$)).subscribe(() => {
    ...
    this.form.submit$.pipe(takeUntil(this.destroy$)).subscribe(() => {
      this.submit();
    });
  ...
  calcTitle(premium: boolean, fide: boolean): string {
    if (fide && premium) {
      return 'PAYGATE.CLUB_PRO_ACCESS';
    }

    if (fide) {
      return 'PAYGATE.PRO_ACCESS';
    }

    if (premium) {
      return 'PAYGATE.CLUB_ACCESS';
    }
  }
  ...
  eventDataLayer() {
    const subscriptionSettings = this.paygateService.cart.value;

    window['dataLayerPush']({
      'event': 'wchPurchase',
      'eventCategory': 'Subscription-Purchase',
      'eventAction': this.form.valid ? 'success' : 'fail',
      'eventLabel': subscriptionSettings.products[0].product.name === 'pro-plan-eur' ? 'Club-Package' : 'Pro-Membership',
      'eventValue': subscriptionSettings.total
    });
  }
}
